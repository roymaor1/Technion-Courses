...
bool TestRemove() {
	...
	ASSERT_NO_THROW(graph.Remove(graph.BeginAt("B")));
...
int main(){
	RUN_TEST(TestOperatorSubscript);         // 25 | 3
	RUN_TEST(TestContains);                  // 26 | 3
	RUN_TEST(TestInsert);                    // 27 | 3
	RUN_TEST(TestRemove);                    // 28 | 3
	RUN_TEST(TestIterators);                 // 29 | 4
	RUN_TEST(TestConstIterators);            // 30 | 2
	RUN_TEST(TestConnect);                   // 31 | 4
	RUN_TEST(TestDisconnect);                // 32 | 4
	RUN_TEST(TestCopy);                      // 33 | 4
	return 0;
}